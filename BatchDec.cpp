...
void BatsDecoder::processReceivedPacket(SymbolType *packet, KeyType batchID, int random){
    
    CheckNode* it = batchSet[batchID];

    // if a new batch is received, initialize the batch
    if (it == NULL){
        it = initNewBatch(batchID, random);
    }
...
CheckNode* BatsDecoder::initNewBatch(KeyType batchID, int random) {
    int i,j;
