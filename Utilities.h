...
class DistSampler {
    double accu[MAX_DEGREE+1]; //累积分布函数数组，用于存储累积概率值
    int n;
public:
    ...
    inline int sample(double pos){ // pos in [0 1] 函数接受一个浮点型参数 pos，表示在累积分布函数中的位置
        int lb = 0, ub = n, mi;
		while(ub-lb > 1){
			mi = lb+(ub-lb)/2;
			if(pos < accu[mi])
...
//通过构造函数初始化累积分布函数，在采样函数中使用二分查找法进行采样操作，实现了从度分布中获取度值的功能。
