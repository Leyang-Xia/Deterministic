...
//        inactRedun = (inactRedun > 8)? inactRedun:8;
    ...
    void receivePacket(SymbolType *packet, KeyType batchID, int random){
        nRecPkg++;

        processReceivedPacket(packet,batchID,random);
...
private:
    // sparse matrix
    CheckNode* initNewBatch(KeyType, int);
    ...
    bool solveCY(int);
    bool solveInactVar();
    void processReceivedPacket(SymbolType *packet, KeyType batchID, int random);
};
